...
INPUT_NAME() {
  ...
  then
    ...
    USER_NAME=$(echo $($PSQL "SELECT username FROM users_info WHERE username='$NAME';") | sed 's/ //g')
    if [[ ! -z $USER_NAME ]]
    then
      ...
      #with <username> being a users_info name from the database,
      ...
      USER_ID=$(echo $($PSQL "SELECT user_id FROM users_info WHERE username='$USER_NAME';") | sed 's/ //g')
      USER_NAME=$(echo $($PSQL "SELECT username FROM users_info WHERE user_id='$USER_ID';") | sed 's/ //g')
      GAME_PLAYED=$(echo $($PSQL "SELECT game_played FROM users_info WHERE user_id=$USER_ID;") | sed 's/ //g')
      BEST_GAME=$(echo $($PSQL "SELECT MIN(best_guess) FROM users_info LEFT JOIN games USING(user_id) WHERE user_id=$USER_ID;") | sed 's/ //g')
    ...
    #Your script should randomly generate a number that users_info have to guess
...
SAVE_USER() {
  ...
  CHECK_NAME=$($PSQL "SELECT username FROM users_info WHERE username='$USER_NAME';")
  if [[ -z $CHECK_NAME ]]
  then
    INSERT_NEW_USER=$($PSQL "INSERT INTO users_info(username, game_played) VALUES('$USER_NAME',1);")
  else
    GET_GAME_PLAYED=$(( $($PSQL "SELECT game_played FROM users_info WHERE username='$USER_NAME';") + 1))
    UPDATE_EXIST_USER=$($PSQL "UPDATE users_info SET game_played=$GET_GAME_PLAYED WHERE username='$USER_NAME';")
  fi
...
SAVE_GAME() {
  ...
  USER_ID=$($PSQL "SELECT user_id FROM users_info WHERE username='$USER_NAME';")
  INSERT_GAME=$($PSQL "INSERT INTO games(user_id, best_guess) VALUES($USER_ID, $NUMBER_OF_GUESSES);")
  USER_NAME=$($PSQL "SELECT username FROM users_info WHERE user_id=$USER_ID;")
}
